...
> **Zenn を参考にしています。**
> エディタの 2 ペイン構成、`:::message` 記法、絵文字アイコン、トピックタグなど、記事投稿まわりの UI/UX は [Zenn](https://zenn.dev) から強く着想を得ています。一方で、Zenn のクローンや代替を目指すものではなく、用途は **「社内 wiki + 軽い社内コミュニティ」** に振っており、コミュニティ機能・所属タグ・公開範囲・AI レビューなどは独自に追加しています。
...
本プロジェクトは [Zenn](https://zenn.dev) の UI と機能から強く着想を得た独自実装の OSS です。「言わなければパクリにならないわけではない」と考えているので、影響元を明示しておきます。
